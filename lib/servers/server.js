...
var Server = exports.Server = function(options, logger) {
    var self = this;
        ...
        if(self._settings.backups.hasOwnProperty(bk)) {
            ...
            if(bak.enabled && bak.interval) {
                try {
                    ...
                    self._logger.log('Backup interval for ' + bk + ' is invalid!', 'error');
    ...
    self.on('shutdown::done', function() {
        self._logger.log(self._settings.name + ' has stopped.', 'info');
    ...
    self.on('server::idle', function() {
        ...
        if(self._starting || self._stopping) {
            self._logger.log(self._settings.name + ' went idle while ' + doing + '!', 'error');
            self._logger.log('Killing idle server...', 'error');
            self._server.kill();

            self._logger.log('Output Buffer dump of last ' + self._outputBuffMax + ' lines:', 'debug');
            self._outputBuff.forEach(function(line) {
                self._logger.log(line, 'debug');
            });
        }
    });
};
...
Server.prototype.isRunning = function(emit) {
    this._cmdDone(emit);
    return !!this._server;
};
...
Server.prototype.start = function(emit) {
    ...
    if(self.isRunning(false)) {
        self._logger.log(self._settings.name + ' is already running!', 'info');
        self._cmdDone(emit);
        return false;
        ...
        self._logger.log('Server is starting up...', 'info');
...
Server.prototype.backup = function(emit) {
    ...
    self._logger.log('Backing up server bin to ' + dname + '...', 'info');
    self._logger.log('This may take a while...', 'info');
...
Server.prototype.unhookOutput = function(cb, emit) {
    var self = this;

    self.removeListener('stdout', cb);
...
Server.prototype._updateFile = function(file, uri, callback) {
    ...
    self._logger.log('Downloading updated ' + file + '...', 'info');
