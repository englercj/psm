 ...
 * TODO:
 * - Map backups (time configurable)
 * - Map backup cleanups (time configurable)
 * - Server backups (time configurable)
 * - Log backups (time configurable)
 **/
...
Minecraft.prototype.backupLogs = function(emit) {
    ...
    if(this.isRunning()) {
	log('Cannot backup maps while server is running', 'info');
	this._cmdDone(emit, false);
	return false;
    }
