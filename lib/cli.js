...
var Cli = exports.Cli = function(psm) {
    ...
    this._piping = false;
    this._pipeEcho = function echoStdo(data) {
	...
	pipe: {
		...
		if(cmd == 'open') {
		    if(args.length < 3) {
			log('You must specify a game to open a pipe to');
		    }
		    self._piping = psm.getServer(args[2]);

		    if(self._piping) {
			self._piping.on('stdout', self._pipeEcho);
			self._piping.on('stderr', self._pipeEcho);
			rl.setPrompt(config.prompt, config.prompt.length);
		    } else {
			log('Uninitialized server ' + args[2]);
		    }
		} else if(cmd == 'close') {
		    self._piping.removeListener('stdout', self._pipeEcho);
		    self._piping.removeListener('stderr', self._pipeEcho);
		    self._piping = false;
		    rl.setPrompt(prompt, promptLen);
		} else {
		    log('Unknown param passed to pipe');
		}
		self.emit('cmd::done');
	    }
	},
    ...
    self.start = function() {
		...
		if(self._piping) {
		    if(cmd == 'pipe') {
			cmds.pipe.func(line, args);
		    } else {
			self._piping.cmd(line);
		    }
		} else {
                    if(cmds[cmd]) {
