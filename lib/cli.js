...
var Cli = exports.Cli = function(psm) {
	...
	pipe: {
		...
		if(cmd == 'open') {
		    ...
		    if(self._piping) {
			self._piping.on('stdout', echoStdo);
			self._piping.on('stderr', echoStdo);
			rl.setPrompt('> ', 2);
		    } else {
			log('Uninitialized server ' + args[2]);
		    }
		} else if(cmd == 'close') {
		    self._piping.removeListener('stdout', echoStdo);
		    self._piping.removeListener('stderr', echoStdo);
		    self._piping = false;
