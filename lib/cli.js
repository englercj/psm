...
var Cli = exports.Cli = function(psm) {
    ...
    cmds = {
		...
		if(cmd == 'status') {
		    ...
		    args.splice(0, 1);
		    var status = psm.cmd(args);
		    
		    var sid = args[1],
		    opts = psm.options(sid, '*'),
		    msg = opts.name.helpCmd + ' [' + opts.sid;
		    //format name & aliases
		    if(opts.aliases && opts.aliases.length > 0)
			msg += ', ' + opts.aliases.join(', ');
		    msg += '] - ' + (psm.isRunning(sid) ? 'RUNNING'.running : 'NOT RUNNING'.stopped);
		    ...
		    var sid = args[1],
		    players = psm.cmd([sid, 'getPlayers']);
