var Hook = require('hook.io').Hook,
fs = require('fs'),
path = require('path'),
util = require('util'),
colors = require('colors');

var Logger = exports.Logger = function() {
    Hook.call(this);

    self.once('hook::ready', function() {
	self.config = require(path.join(__dirname, '../config/config.json'));
	colors.setTheme(self.config.colors);

	self.on('*::log::info', function(data) {
	    self.log(data, 'info');
	});

	self.on('*::log::error', function(data) {
	    self.log(data, 'error');
	});

	self.on('*::log::debug', function(data) {
	    self.log(data, 'debug');
	});
    });
};

//inherits from event emmitter
util.inherits(Logger, Hook);

Logger.prototype.reloadConfig = function(cb) {
    var self = this;

    try {
	self.config = require(path.join(__dirname, '../config/config.json'));
	colors.setTheme(self.config.colors);

	if(cb) cb(null);
    } catch(e) {
	if(cb) cb(e);
    }
};

//Log worker
Logger.prototype.log = function(msg, type) {
    var threshold, error = false, head = '[';

    switch(type) {
    case 'error':
        threshold = 0;
        head += 'ERROR'.error;
        error = true;
        break;
    case 'debug':
        threshold = 2;
        head += 'DEBUG'.debug;
        break;
    case 'info':
        head += 'INFO'.info;
    default:
        threshold = 1;
        break;
    }

    if(type)
        head += '] ';
    else
        head = '';

    if(config.logging.level >= threshold) {
	//emit this message
	/*
	if(emit !== false) {
            if(typeof(msg) == 'string') {
		this.emit('log::msg', head + msg);
            } else {
		this.emit('log::msg', head);
		this.emit('log::msg', msg);
            }
	}
	
	//check if we force to console
	if(forceStdout) {
            if(typeof(msg) == 'string') {
		console.log(head + msg);
            } else {
		console.log(head);
		console.log(util.inspect(msg, false, 5));
            }
	}
	*/
	//Log to file
        var d = new Date(),
        date = (d.getMonth() + 1) + '-' + d.getDate() + '-' + d.getFullYear(),
        time =  d.getHours() + ':' + d.getMinutes() + ':' + d.getSeconds(),
        fname = path.join(__dirname, '..', config.logging.dir, 'log_' + date + '.log');

        head += '(' + time + ') ';

        fs.open(fname, 'a', '0666', function(err, fd) {
            if(typeof(msg) == 'string') {
                fs.write(fd, (head + msg).stripColors + '\n', null, 'utf8', function() { fs.close(fd); });
            } else {
                fs.write(fd, head.stripColors, null, 'utf8', function() {
                    fs.write(fd, util.inspect(msg, false, 5), null, 'utf8', function() { fs.close(fd); });
                });
            }
        });

        //if its an error, log it seperately
        if(error) {
            fname = path.join(__dirname, '..', config.logging.dir, 'error_' + date + '.log');
            fs.open(fname, 'a', '0666', function(err, fd) {
                if(typeof(msg) == 'string') {
                    fs.write(fd, (head + msg).stripColors + '\n', null, 'utf8', function() { fs.close(fd); });
                } else {
                    fs.write(fd, head.stripColors, null, 'utf8', function() {
                        fs.write(fd, util.inspect(msg, false, 5), null, 'utf8', function() { fs.close(fd); });
                    });
                }
            });
        }
    }
};