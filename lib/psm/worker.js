...
var Worker = exports.Worker = function(o) {
    var self = this;
...
Worker.prototype._notifyMessage = function(data, cb) {
    var self = this,
    opts = {
	url: 'http://localhost:' + self._managerPort + '/worker/message/' + self._serverId +
	    '?token=' + self._managerToken,
	method: 'POST',
	json: data
    };
...
Worker.prototype._startServer = function(cb) {
	...
	reloadConfig: scopify(self, self.reloadConfig),
...
Worker.prototype._setupOutputHandlers = function() {
    var self = this;
	...
	self._notifyMessage({ channel: 'output', message: { lines: lines } });
    });

    self.outputs.on('player::connect', function(name) {
	self._notifyMessage({ channel: 'player::connect', message: { name: name } });
    });

    self.outputs.on('player::disconnect', function(name) {
	self._notifyMessage({ channel: 'player::disconnect', message: { name: name } });
    });

    self.outputs.on('player::chat', function(timestamp, name, msg) {
	self._notifyMessage({ channel: 'player::chat', message: { timestamp: timestamp, name: name, msg: msg } });
    });
};
