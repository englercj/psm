...
var Worker = function() {
	...
	if(err) { self._notifyError(err); process.exit(1); return; }

	self._getSettings(function(err) {
	    if(err) { self._notifyError(err); process.exit(1); return; }
...
Worker.prototype._getSettings = function(cb) {
	...
	if(!err && res.statusCode == 200) {
	    ...
	    cb(err ? err : new Error('Got non 200 status code (' + res.statusCode + ')'));
	}
    });
};
