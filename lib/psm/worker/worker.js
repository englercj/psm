...
Worker.prototype.startup = function(server, cb) {
	...
	self.outputs = new helpers.output.OutputParser({
            logger: psm.log
	});
	self._setupOutputListeners();
	
	psm.log.silly('Instantiating FileManager');
	self.files = new helpers.files.FileManager({
            logger: psm.log,
            worker: self,
	...
	self.proc = new helpers.proc.ProcManager({
            logger: psm.log,
	...
	self.plugins = new helpers.plugins.PluginManager({
	    logger: psm.log,
	    outputParser: self.outputs,
	    files: self.files
	});
	...
	self.worlds = new helpers.worlds.WorldManager({
	    logger: psm.log,
	    outputParser: self.outputs,
	    files: self.files
	});
	...
	self.jobs = new helpers.jobs.JobManager({
	    logger: psm.log,
	    outputParser: self.outputs,
	    files: self.files
	});
