...
/*Manager.prototype.daemonize = function() {
    var child = forever.startDaemon(path.join(__dirname, 'manager.js'), {
	max: 1,
...
Manager.prototype._startServer = function(cb) {
    var self = this,
    spath = psm.config.get('sockets:manager'),
    f;

    psm.log.silly('Checking for existance of socket.');
    try { f = fs.lstatSync(spath); } catch(e) {}

    if(f) {
	//socket exists, GET THE FUCK OUT
	psm.log.error('Socket already exists, unable to start enode server!');
	cb(new Error('Socket already exists, unable to start enode server!'));
	return;
    }

    psm.log.silly('Creating enode instance.');
    self._server = new enode.Server({
