...
Manager.prototype.stop = function(server, remote, cb) {
    ...
    self._proxyToWorker('stop', [], server, remote, function() {
        worker.proc.stop().once('stop', function() {
...
Manager.prototype.workerReady = function(server, cb) {
    ...
    var client = new enode.Client().connect(worker.socketFile, function(err, sock, conn) {
        if(err) {
            ...
            psm.log.debug('Worker socket connected.');
            worker.socket = sock;
...
Manager.prototype.workerError = function(server, err, cb) {
    ...
    psm.log.error('Worker Error on server %s: %s', server, err);
...
Manager.prototype._proxyToWorker = function(fn, args, server, remote, cb) {
    var self = this;
    ...
    if(remote) {
	    ...
	    args.push(cb);
            worker.socket[fn].apply(worker.socket, args);
        }
    }
};
