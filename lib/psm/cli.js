...
app.cmd(/import (.+)/, cli.importServers = function() {
	...
	psm.log.debug('Attempting to import new server to remote ' + server.remote, server);
	psm.manager.serverAdd(server, remote, function(err) {
	    if(err) {
		psm.log.error('Unable to add server', err);
	    } else {
		psm.log.info('New server added to ' + (server.remote ? 'remote ' + server.remote : 'local manager'));
	    }
	});
    } catch(e) {
	psm.log.info('Unable to add server', e);
    }
});
