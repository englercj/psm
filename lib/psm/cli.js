...
help = [
    'usage: psm ACTION [options] SERVER',
    '',
    ...
    '  list             Lists all manages servers',
...
app.cmd(/start (.+)/, cli.startServer = function() {
    ...
    psm.log.silly('Got start command for server: ' + server);

    psm.manager.startServer(server);
});

app.cmd(/stop (.+)/, cli.stopServer = function() {
    var server = app.argv._[1];

    psm.log.silly('Got stop command for server: ' + server);

    psm.manager.stopServer(server);
});

app.cmd(/restart (.+)/, cli.restartServer = function() {
    var server = app.argv._[1];

    psm.log.silly('Got restart command for server: ' + server);

    psm.manager.stopServer(server, function() {
	psm.manager.startServer(server);
    });
});
...
app.cmd(/import (.+)/, cli.importServers = function() {
    ...
    psm.log.silly('Got import command for file: ' + path.resolve(file));

    psm.manager.importServers(file);
});

app.cmd(/show (.+)/, cli.showServerInfo = function() {
    var server = app.argv._[1],

    psm.log.silly('Got show command for server: ' + server);

    psm.manager.getServerInfo(server, function(info) {
	//TODO: Parse info object
	psm.log.info(info);
    });
});

app.cmd(/attach (.+)/, cli.attachToServer = function() {
    var server = app.argv._[1];

    psm.log.silly('Got attach command for server: ' + server);
...
app.cmd('list', cli.list = function() {
    ...
    psm.manager.getServers(function(servers) {
	//TODO: Parse servers
