...
var FileManager = exports.FileManager = function() {
    var self = this;
...
FileManager.prototype.worldsToRam = function(cb) {
    ...
    worlds.forEach(function(world) {
	if(ramWorlds.indexOf(world) != -1) {
	    utle.cpr(path.join(worldDisk, world), path.join(worldRam, world), function(err) {
		if(err) errors.push(err);
...
FileManager.prototype.checkWorldLinks = function(cb) {
    ...
    if(bin == worldDisk) { if(cb) cb(null); return; }
...
self._doCreateLoc = function(p, world, l, lLoc) {
    var worldLoc = path.join(p, world);
    
    utile.mkdirp(worldLoc, function(err) {
	//ensure lLoc points to ram location
	if(lLoc != worldLoc) {
	    if(l) fs.unlink(worldLoc, function(err) {
		if(err) { if(cb) cb(err); return; }
		
		fs.symlink(worldLoc, path.join(bin, world), function(err) {
		    if(err) { if(cb) cb(err); return; }
		    
		    self.log.debug('Created symlink for ' + world + ' for ' + worldLoc);
		});
	    });
	}
    });
...
FileManager.prototype.worldsToRam = function() {

};

FileManager.prototype.worldsToDisk = function() {
...
FileManager.prototype._updateFile = function(file, uri, cb) {
    ...
    var req = request(uri, function() {
	    ...
	    cb(new Error('The file failed to download, unable to find new file ' + fileNew));
        }
    });
