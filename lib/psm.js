...
psm.init = function(cb) {
    Config.findOne({}, function (err, doc) {
        if(err) {
	    console.log('Unable to find PSM Configuration.');
	    console.log('Please check your database settings.');
            if(cb) cb(err);
            return;
        }
...
psm._configManager = function() {
    psm.manager = {
        cmd: function(cmd, server, data, cb) {
	    ...
	    var opts = {
		url: 'http://localhost:' + psm.config.api.port +
                    '/' + cmd + (server ? '/' + server : '') +
		    '?token=' + psm.config.token,
		method: (cmd == 'add' ? 'POST' : 'GET'),
		json: (cmd == 'add' ? data : undefined)
	    };
