/**
 * psm.js: Include for the Panther Server Manger module
 *
 * (c) 2012 Panther Development
 * MIT LICENCE
 *
 **/

//////////
// Required Includes
///////////////////////////
var fs = require('fs'),
path = require('path'),
events = require('events'),
cp = require('child_process'),
winston = require('winston'),
utile = require('utile'),
nconf = require('nconf'),
dnode = require('dnode'),
pkginfo = require('pkginfo');

//////////
// Setup exports
///////////////////////////
var psm = exports;

//////////
// Setup PSM Components
///////////////////////////
psm.initialized = false;

psm.configFile = path.join(__dirname, '../config/config.json');
psm.serversFile = path.join(__dirname, '../config/servers.json');

psm.config = new nconf.File({ file: psm.configFile });
psm.servers = new nconf.File({ file: psm.serversFile });

psm.config.loadSync();
psm.servers.loadSync();

//change sockets dir if this is windows
if(process.platform === 'win32') {
    psm.config.set('sockets:dir', '\\\\.\\pipe\\');
}

psm.manager = getManager();
psm.cli = require('./psm/cli');

//////////
// Setup winston
///////////////////////////
var logLevels = {
    levels: {
	silly: 0,
	debug: 1,
	verbose: 2,
	info: 3,
	warn: 4,
	error: 5,
	silent: 6
    },
    colors: {
	silly: 'grey',
	debug: 'green',
	verbose: 'magenta',
	info: 'cyan',
	warn: 'yellow',
	error: 'red'
    }
};

//Setup winston loggers
psm._console = new winston.Logger({
    level: psm.config.get('cli:logLevel'),
    levels: logLevels.levels,
    handleExceptions: true,
    transports: [
	new winston.transports.Console({
	    level: psm.config.get('cli:logLevel'),
	    levels: logLevels.levels,
	    handleExceptions: true,
	    colorize: true
	})
    ]
});

psm._console.cli();

psm._file = new winston.Logger({
    level: psm.config.get('fileLog:level'),
    levels: logLevels.levels,
    transports: [
	new winston.transports.File({
	    level: psm.config.get('fileLog:level'),
	    levels: logLevels.levels,
	    filename: path.join(psm.config.get('fileLog:dir'), 'psm.log'),
	    timestamp: true
	})
    ]
});

//wrap loggers into my own object, so I can keep them seperate
//but sitll call both via a single function
psm.log = {
    log: function() {
	psm._console.log.apply(psm._console, arguments);
	psm._file.log.apply(psm._file, arguments);
    }
};
utile.each(logLevels.levels, function(val, level, o) {
    psm.log[level] = function() {
	psm._console[level].apply(psm._console, arguments);
	psm._file[level].apply(psm._file, arguments);
    }
});

//add custom colors
winston.addColors(logLevels.colors);

//////////
// Setup PkgInfo Version
///////////////////////////
require('pkginfo')(module, 'version');

//////////
// Helpers
///////////////////////////
function getManager(cb) {
    var s = path.join(psm.config.get('sockets:dir'), 'psm-manager.sock');
    path.exists(s, function(exists) {
	if(exists) {
	    dnode.connect(s, function(remote) {
		cb(remote);
	    });
	} else {
	    var m = cp.fork(path.join(__dirname, '../../bin/psm-manager'));
	    m.once('message', function(msg) {
		if(msg == 'ready') {
		    getManager(function(man) { cb(man); });
		} else {
		    psm.log.error('Unable to launch manager!');
		}
	    });
	}
    });
};

//TODO: Defaults for config
/*
psm.load = function() {
    if(psm.initialized) return;

    var config;

    try {
	config = psm.config.loadSync();
    } catch(e) {
	psm.error('Unable to load log file!', e);
	return false;
    }

    //ensure we have the minimun config
    options = options || {};

    //Setup default options
    options = {
	cli: { logLevel: 'info', prompt: '> ' },
	api: { enabled: true, host: '127.0.0.1', port: 8596 },
	debug: false
    };

    

    //Ensure Update config and save
    psm.config.set('cli', config.cli);
    psm.config.set('api', config.api);
    psm.config.set('debug', config.debug);

    psm.config.save();
};
*/