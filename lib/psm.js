...
Psm.prototype.reloadConfig = function(newConfig) {
    var self = this;

    self.emit('config::reload', newConfig);
};
...
Psm.prototype.reloadServers = function(newServers, emit) {
    ...
    if(self.anyRunning()) {
	self._cmdDone('Cannot reload server list with servers running!');
    } else {
	//TODO: Do reload
    }
};
...
Psm.prototype._loadServerModule = function(sets) {
    ...
    self._logger.log('Attempting to load Module using rname: ' + rname + ', mod: ' + mod, 'debug');
    try {
	...
	self._logger.log('Unable to load server module for "' + sets.type + '"', 'error');
	self._logger.log('Attempted file "' + rname + '" with class "' + mod + '"', 'error');
