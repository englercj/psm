...
Psm.prototype.anyRunning = function() {
	...
	if(self.servers.hasOwnProperty(srv)) {
	    if(self.servers[srv].isRunning()) return true;
	}
    }
};
...
Psm.prototype.isRunning = function(server) {
    return (this.servers[server] && this.servers[server].isRunning());
};
...
Psm.prototype.option = function(server, option, value) {
    if(this.settings.servers[server]) {
	if(typeof(option) == 'string') {
	    var levels = option.split('.'),
	    opt = this.settings.servers[server],
	    i = levels.length - 1,
	    key;
